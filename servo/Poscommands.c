...
/* USER CODE BEGIN PFP */
void Pos_command_write(uint8_t id, uint16_t position) {
...
void Pos_command_read(uint8_t id, uint16_t position) {
...
int main(void)
{
  ...
  while (1)
  {
	...
	goalPosition = centerPosition + (int)(sin(radiansval) * maxPosition);
	  // サーボにコマンドを送信
	Pos_command_write(servoID, goalPosition);
