...
namespace run
{
    constexpr uint32_t kDefaultStepHz = 300U;

    // タイマ本体, タイマch, IOポート, pin番号, 正方向のときにDIRをHIGHにするか, モーター1ステップ角(def 1.8), マイクロステップ倍率(pulse/revの1/200), 減速比） 
    StepAxis axis1{&htim2, TIM_CHANNEL_1, GPIOB, GPIO_PIN_7, true, 1.8f, 2U, 15.0f}; // STEP=PA15, DIR=PB7
    StepAxis axis2{&htim3, TIM_CHANNEL_1, GPIOA, GPIO_PIN_4, true, 1.8f, 2U, 15.0f}; // STEP=PA6,  DIR=PA4
    StepAxis axis3{&htim4, TIM_CHANNEL_1, GPIOB, GPIO_PIN_4, true, 1.8f, 2U, 15.0f}; // STEP=PB6,  DIR=PB4

    StepAxis* const axes[] = {&axis1, &axis2, &axis3};
    constexpr size_t kAxisCount = sizeof(axes) / sizeof(axes[0]);

    void move_axis_to_deg(size_t axis_index, float target_deg)
    {
        if (axis_index >= kAxisCount) return;
        axes[axis_index]->moveToDeg(target_deg);
    }

    void on_step_pulse_finished(TIM_HandleTypeDef* htim)
    {
        for (size_t i = 0; i < kAxisCount; ++i) {
            StepAxis& axis = *axes[i];
